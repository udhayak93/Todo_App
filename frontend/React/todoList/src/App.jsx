...
function App() {
  ...
  const API_URL = (import.meta.env.VITE_API_URL || "http://localhost:3030").replace(/\/$/, "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (listItem._id) {
        await apiRequest(`/updateList/${listItem._id}`, {
          method: "PUT",
          body: JSON.stringify({ title, description }),
        });
        alert("Updated successfully!");
      } else {
        await apiRequest("/addList", {
          method: "POST",
          body: JSON.stringify({ title, description }),
        });
  ...
  const handleDelete = async (task) => {
    ...
    try {
      setError("");
      await apiRequest(`/deleteList/${task._id}`, {
        method: "DELETE",
      });
    ...
    <div className="app">
      <section className="card card-1">
        <div className="cardHeader createHeader">
          ...
          <div>
            <h1>{listItem._id ? "Update Work" : "Create Work"}</h1>
        ...
        <form className="form" onSubmit={handleSubmit}>
          ...
          <button className="submitBtn" type="submit" disabled={saving}>
            ➤ &nbsp; {saving ? "Saving..." : listItem._id ? "Update" : "Submit"}
          </button>
        </form>
      </section>
      ...
      <section className="card">
        ...
        {loading ? (
          <p className="empty">Loading works...</p>
        ) : todoList.length === 0 ? (
          <p className="empty">No works available</p>
        ) : (
          todoList.map((task) => (
